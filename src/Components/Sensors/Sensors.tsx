...
export default function Sensors (props: ISensorsProps) {
  ...
  useEffect(() => {
    ...
    Promise.all(dataFetchPromises).then((r) => {
      console.log(r)
      const empty: string[] = []
      const nonEmpty: string[] = []
      r.forEach(sensorResult => {
        ...
        if (y !== undefined) {
          ...
          if (sensorIsEmpty) {
            empty.push(sensorResult.id)
          } else {
            nonEmpty.push(sensorResult.id)
          }
        }
      })
      setEmptySensors(empty)
      setNonEmptySensors(nonEmpty)
    }).catch(error => console.error(error))
  }, [sensors, datasetId, station])
